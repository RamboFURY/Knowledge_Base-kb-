...
// Function to get all posts (approved and unapproved)
  ...
  {
    global $dblink;
    $query = $dblink->prepare("SELECT login_credentials.name, post_id ,title, description, resolution, approved, creation_time FROM posts INNER JOIN login_credentials ON posts.user_id=login_credentials.id ORDER BY creation_time");
    $query->execute();
    return ($query->get_result())->fetch_all(MYSQLI_ASSOC);
  }
